...
Snake.prototype.init=function(){
    ...
    //给蛇添加一条相互新，用来表示蛇走的方向
    this.direction=this.directionNum.right;//默认让设往右走
};
...
Snake.prototype.getNextPos=function(){
    ...
    this.pos.forEach(function(value){
        if(value[0]==nextPos[0] && value[1]==nextPos[1]){
            //如果数组中的两个数据都相等，就说明下一个点再蛇身上里面能找到，代表撞到自己了
            selfCollied=true;
        }
    });
    if(selfCollied){
        ...
        this.stragtegies.die.call(this);
        return;
    }
    ...
    if(food && food.pos[0]==nextPos[0] && food.pos[1]==nextPos[1]){
        ...
        console.log('撞到食物了！')
        this.stragtegies.eat.call(this);
        return;
    }
    

    //下个点什么都不是，走
    this.stragtegies.move.call(this);
};

//处理碰撞后要做的事
Snake.prototype.stragtegies={
    move:function(format){//这个参数用于决定要不要删除最后一个方块（蛇尾），当传了这个参数后就表示要吃。
        //创建新身体（在旧蛇头的位置）
    ...
    eat:function(){
        this.stragtegies.move.call(this,true);
        createFood();
        game.score++;
    },
    die:function(){
...
function createFood(){
    ...
    food.pos=[x,y]; //存储一下生成食物的坐标，用于蛇头要走的下一个点做对比
...
Game.prototype.init=function(){
    ...
    document.onkeydown=function(ev){
        if(ev.which==37 && snake.direction!=snake.directionNum.right ){ //用户按下左键的时候，这条蛇不能是正在往右走
